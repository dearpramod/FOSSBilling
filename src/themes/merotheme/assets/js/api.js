/**
 * Stablepanel API for JavaScript.
 *
 * @copyright Stablepanel (https://stablepanel.com)
 * @license   Apache-2.0
 *
...
// ─── legacy shims kept for backward-compatibility with existing themes ────────
// These delegate to the new Tools helpers so existing calls still work.
FormData.prototype.serialize = function () {
    if (!this.get('CSRFToken')) {
        this.append('CSRFToken', Tools.getCSRFToken());
    }
    return Tools.serializeFormData(this);
};
FormData.prototype.serializeObject = function () {
    return Tools.serializeFormDataToObject(this);
};
FormData.prototype.serializeJSON = function () {
    return Tools.serializeFormDataToJSON(this);
};
// ─────────────────────────────────────────────────────────────────────────────
...
const Tools = {
  ...
  /**
   * Parses and validates the data-fb-api JSON attribute value from a DOM element.
   *
  ...
  parseDataAttr: function (dataAttrValue) {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'href')) { assertString(data.href, 'href'); }
    if (Object.prototype.hasOwnProperty.call(data, 'type')) { assertString(data.type, 'type'); }
    if (Object.prototype.hasOwnProperty.call(data, 'endpoint')) { assertString(data.endpoint, 'endpoint'); }
    if (Object.prototype.hasOwnProperty.call(data, 'callback')) { assertString(data.callback, 'callback'); }
    if (Object.prototype.hasOwnProperty.call(data, 'message')) { assertString(data.message, 'message'); }
    if (Object.prototype.hasOwnProperty.call(data, 'redirect')) { assertString(data.redirect, 'redirect'); }
    if (Object.prototype.hasOwnProperty.call(data, 'reload')) { assertBoolean(data.reload, 'reload'); }
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'modal')) {
      ...
      ['title', 'content', 'button', 'buttonColor', 'label', 'value', 'key'].forEach((field) => {
        if (Object.prototype.hasOwnProperty.call(modal, field)) { assertString(modal[field], `modal.${field}`); }
      });
    }
  ...
  serializeFormData: function (formData) {
    ...
    if (!formData.has('CSRFToken')) {
      ...
      if (token) { params.append('CSRFToken', token); }
    }
   ...
   * Handles array fields (name[]) and nested bracket notation (name[key]).
   *
   * @param {FormData} formData The FormData object to serialize.
  ...
  /**
   * Converts a FormData object into a JSON string.
   *
   * @param {FormData} formData The FormData object to serialize.
...
/**
 * Creates an API wrapper for a specific role (admin, client, guest).
 *
...
/**
 * Stablepanel API wrapper for JavaScript.
 */
const API = {
   ...
   * @param {boolean}         [enableLoader=true]
  ...
  makeRequest: function (method, url, params, successHandler, errorHandler, enableLoader = true, timeoutMs = 30000) {
    ...
    // Inject CSRF token into params
    if (isFormData) {
      if (!params.has('CSRFToken')) { params.append('CSRFToken', Tools.getCSRFToken()); }
    ...
    // Always send X-CSRF-Token header for server-side header-based verification
    const headers = {
    ...
    return fetch(url.toString(), {
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          ...
          if (contentType.includes('application/json')) {
            return response.json().then(
              (body) => {
                const err = new Error(body?.error?.message || `HTTP error ${status}: ${statusText}`);
        ...
        if (contentType && contentType.includes('application/json')) {
          return response.json().catch(() => { throw new Error('Invalid JSON response from server'); });
        } else {
          return response.text().then(text => {
            try { return JSON.parse(text); }
            catch (e) { throw new Error('Invalid or non-JSON response from server'); }
          });
        }
      })
      .then((response) => {
        ...
        if (typeof successHandler === 'function') { successHandler(response ? response.result : undefined); }
      ...
      .catch((error) => {
        ...
        if (error.name === 'AbortError') {
          errorObj = { message: 'Request timed out after ' + (timeoutMs / 1000) + ' seconds', code: 'timeout_error' };
        } else if (error.name === 'TypeError' && error.message.includes('NetworkError')) {
          errorObj = { message: 'Network connection error', code: 'network_error' };
        } else {
          errorObj = { message: error.message || 'Unknown error occurred', code: error.code || 'unknown_error' };
        }

        // Codes 201-206 = session expired / bad credentials; 1002 = unauthorized IP; 1004 = bad referrer.
        // Show a brief toast then reload — the server will redirect to the login page.
        const authFailureCodes = [201, 202, 203, 204, 205, 206, 1002, 1004];
        if (authFailureCodes.includes(Number(errorObj.code))) {
          if (typeof FOSSBilling !== 'undefined' && typeof FOSSBilling.message === 'function') {
            FOSSBilling.message('Your session has expired. Redirecting to login…', 'error');
          }
        ...
        if (typeof errorHandler === 'function') {
          // Expected / handled error — log at info level so devtools stay clean.
          console.info(`API handled error: ${errorObj.message}`);
          errorHandler(errorObj);
        } else {
          // Truly unhandled — log as error so it is visible.
          console.error(`API unhandled error: ${errorObj.message}`);
      ...
      .finally(() => {
        if (enableLoader && loader) {
          if (loader._fadeInTimeout) { clearTimeout(loader._fadeInTimeout); }
  ...
  /**
   * Called after a successful API request to handle redirect / reload / message / callback.
   *
  ...
  _afterComplete: function (object, result) {
    ...
    if (apiData.hasOwnProperty('redirect')) {
      // Security: only allow same-origin redirects to prevent open-redirect abuse.
      try {
        const target = new URL(apiData.redirect, window.location.origin);
        if (target.origin !== window.location.origin) {
          console.error('Blocked cross-origin redirect in data-fb-api:', apiData.redirect);
          return;
        }
        window.location.assign(target.toString());
      } catch (e) {
        console.error('Invalid redirect URL in data-fb-api:', apiData.redirect);
      }
      return;
    }
  ...
  /**
   * Creates and appends a loading spinner to the document body.
   *
  ...
  /**
   * Attach submit listeners to <form data-fb-api> elements.
   */
  _apiForm: function () {
    ...
    if (formElements.length > 0) {
      formElements.forEach(formElement => {
        formElement.addEventListener('submit', function (event) {
          ...
          const toggleButtons = (disable) => { buttons.forEach(button => button.disabled = disable); };
          toggleButtons(true);
          ...
          API.makeRequest(
            ...
            data,
            (result) => { toggleButtons(false); API._afterComplete(formElement, result); return result; },
            (error) => { toggleButtons(false); FOSSBilling.message(`${error.message} (${error.code})`, 'error'); }
          );
        });
      });
    }
  ...
  /**
   * Attach click listeners to <a data-fb-api> elements.
   */
  _apiLink: function () {
    ...
    if (linkElements.length > 0) {
      linkElements.forEach(linkElement => {
        linkElement.addEventListener('click', function (event) {
          ...
          if (!apiData.href && (!rawHref || rawHref === '#')) { return; }
          ...
          if (apiData.hasOwnProperty('modal') && apiData.modal.type === 'prompt') {
            ...
            Modals.create({
              ...
              confirmCallback: () => { handleApiRequest('GET', linkElement.getAttribute('href')); },
            });
          } else {
