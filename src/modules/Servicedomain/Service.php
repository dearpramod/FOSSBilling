...
class Service implements \FOSSBilling\InjectionAwareInterface
{
    ...
    /**
     * Read WHOIS contact details from the local DB and return them keyed by role.
     * Since ConnectReseller assigns one contact to all four roles, each role
     * returns the same contact_* column values.
     *
     * @return array{registrant: array, admin: array, tech: array, billing: array}
     */
    public function getWhoisInfo(\Model_ServiceDomain $model): array
    {
        $contact = [
    ...
    /**
     * Fetch live WHOIS contact data from the registrar, persist it to the DB,
     * and return it structured by role.
     *
     * Only the contact_* columns are updated — expiry/nameserver fields are
     * handled by the full syncWhois() cron path.
     *
     * @return array{registrant: array, admin: array, tech: array, billing: array}
     */
    public function syncWhoisInfo(\Model_ServiceDomain $model): array
    {
        // @adapterAction
        [$domain, $adapter] = $this->_getD($model);
        $whois = $adapter->getDomainDetails($domain);
        ...
        if ($contact !== null) {
            ...
            $this->di['logger']->info('Synced WHOIS contacts for domain #%s from registrar', $model->id);
        }

        return $this->getWhoisInfo($model);
    }
