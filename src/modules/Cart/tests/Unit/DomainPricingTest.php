...
test('cartProductToApiArray uses resolved initial domain term pricing', function (): void {
    ...
    $extensionService->shouldReceive('isExtensionActive')->with('mod', 'partnership')->andReturn(false);

    $di = container();
    $di['em'] = $emMock;
    $di['mod_service'] = $di->protect(function (string $serviceName) use ($productService, $extensionService) {
