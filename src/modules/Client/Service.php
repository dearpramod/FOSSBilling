...
class Service implements InjectionAwareInterface
{
     ...
     *
     * Returns the legacy \Model_Client (not the Doctrine entity) since callers —
     * currently only the wholly custom Sociallogin module, which has no upstream
     * equivalent to migrate alongside — still consume it via RedBean property access.
     *
    ...
    public function oauthCreateClient(array $data): \Model_Client
    {
        ...
        $this->di['events_manager']->fire(['event' => 'onAfterClientSignUp', 'params' => $event_params]);
        $this->di['logger']->info('Client #%s signed up via OAuth', $client->getId());

        return $this->di['db']->getExistingModelById('Client', (int) $client->getId(), 'Client not found');
    }
